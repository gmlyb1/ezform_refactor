	...
	<script type="text/javascript">
		function remove_bdchk(data) {
			if (!confirm("삭제 하시겠습니까?")) return false;
				...
				location.href="/ezform/ez_inq/remove?inq_no="+data;
			}
		}
		...
		function remove_comchk(data1, data2) {
			if (!confirm("삭제 하시겠습니까?")) return false;
				...
				location.href="/ezform/comment/remove?inq_no="+data1+"&inq_no="+data2;
			}
		}
		function img_view(image) {
			// 새 창으로 이미지 띄우기
			var url = "/ezform/ez_inq/viewImg?imgName="+image;
                           ...
                           <th>
                        	  ...
                        	  <div class="d-flex align-items-center">
                        	 	<div class="mb-3 col-xl-4">
								  ...
								  <p class="h6">${vo.inq_dname }</p>
							    </div>
								<div class="mb-3 col-xl-7">
	                              <label class="form-label">이름</label>
							      <p class="h6">${vo.inq_name}</p>
								</div>

                        	  </div>  	 
                              <div class="mb-3">
                                <label class="form-label">제목</label>
								<p class="h6">${vo.inq_title}</p>
							  </div>	
							  <!-- 이미지 형태로 수정해보기 -- 안되면 다운로드 형태로 -->
                              <div class="mb-3">
                                 <label class="form-label">이미지 보기</label>
                               <c:choose>
			                   		<c:when test="${vo.inq_file != null }">
			                   			<a href="javascript:img_view('${vo.inq_file }');"><p>${vo.inq_file }</p></a>
			                   		</c:when>
			                   		<c:otherwise>
			                   			<p class="h6">이미지 없음</p>
			                   		</c:otherwise>
                              ...
                              <hr>
								...
								<c:otherwise>
									<div class="mb-3" style="text-align:center;">
										<button type="button" class="btn btn-primary btn-sm" name="like" onclick="location.href='/ezform/board/like?inq_no=${vo.inq_no}';">
								 ...
								 <form role="form" method="post" action="/ezform/comment/add">
								    <div class="d-flex align-items-center">
									    <p>
										   <textarea class="form-control" rows="3" cols="155" name="inq_content" placeholder="댓글을 남겨주세요" style="resize: none;"></textarea>
										</p>
										<input type="hidden" name="inq_no" value="${vo.inq_no }"/>
										<p>				
